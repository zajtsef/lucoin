...
#if defined(NDEBUG)
# error "Lucoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x684c46f057092b30a2efc6f864022945c2afe2b0b68a79856f6b90fad922c98d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Lucoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Lucoin Signed Message:\n";
...
{
    // Lucoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Lucoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Lucoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1502898912, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "ljojbojboib pjrgpierjbkpti pojkrgpoerjkp";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040c6e244aab9d18c03d86815608b7203f70803280cc75c838d7c537d0b62bfb93e1f6a641967bc685648daebeaa46ee167631ad79d49f118d19734aa6de7f5bfc") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1502898912;
        ...
        assert(block.hashMerkleRoot == uint256("0x78c611e646497429935cef270b0d6084ba8202e6cb5417526fb8692a18fec58e"));
...
unsigned char pchMessageStart[4] = { 0xea, 0x4a, 0x13, 0x10 }; // Lucoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LucoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LucoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LucoinMiner(CWallet *pwallet)
{
    printf("LucoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lucoin-miner");
    ...
    try { loop {
        ...
        printf("Running LucoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LucoinMiner, pwallet));
}
